...
def longSentenceBiDir(gramlist, words, n = 50):
    gramlen = len(gramlist)
    sentenlen = len(words)
...
def genePointBidir(choices,word_to_fix):
    metaphone = Metaphone()
    wordlist = []
    pointList = [[]]
    for choice in choices:
    ...
    for i in range(len(testtxt)):
        if testtxt[i] != testtxtr[i]:
            if i <= (max_gram - 1):
                ...
                word_to_correct = testtxt[i]
            print("To fixing: ", current_word,word_to_correct)
            choice = longSentence(gramlist, current_word, 1)
            resu,confi = genePoint(choice,word_to_correct)
